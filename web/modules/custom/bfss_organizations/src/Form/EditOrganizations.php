...
class EditOrganizations extends FormBase {
     ...
     if($Organizations_permissions['edit']==1 || $Organizations_permissions['admin']==1){
        if( $param['nids'] != '[]'){
            ...
            $form['resident'] = [
              ...
              if(is_array($query_nids) && !empty($query_nids)){
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $query_nids = !empty(json_decode($param['nids'])) ? json_decode($param['nids']) : '';
    if(is_array($query_nids) && !empty($query_nids)){
        if(!empty($form_state->getValues('resident')['resident']) ){//for multiple save org
                    $data=[];
      ...
      if(isset($param['nids'])){
                    ...
                    $node = Node::load($param['nids']);
                    $node->field_address_1->value = $form_state->getValue('address_1');
                    $node->field_address_2->value = $form_state->getValue('address_2');
                    $node->field_city->value = $form_state->getValue('city');
                    $node->field_state->value =  $form_state->getValue('state'); 
                    $node->field_zip->value = $form_state->getValue('zip'); 
                    $node->field_organization_name->value = $form_state->getValue('organization_name'); 
                    $node->field_type->value =  $form_state->getValue('type');  
                    //$node->title->value = $value['type'].'-'.$value['organization_name'];
                    //$node->setPublished(FALSE);
                    $node->save();
      }
